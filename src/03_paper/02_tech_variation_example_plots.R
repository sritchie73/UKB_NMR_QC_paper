...
ala <- dat[variable == "Ala", .(sample_id, visit, raw)]
...
ala <- ala[!is.na(raw), .(metric=names(summary(raw)), value=as.vector(summary(raw))), by=.(spectrometer, plate_id, plate_MEASURED_DATE)]
...
g1 <- ggplot(ala, aes(x=factor(plate_order), color=factor(spectrometer), fill=factor(spectrometer))) + 
  ...
  scale_fill_calc() +
  ylab("Alanine (mmol/L)") + xlab("Plate") +
  theme_bw() +
...
ggsave(g, width=2.8, height=1.4, file="paper_output/ala_inter_plate_variation.pdf")

# Plot histidne vs. sample degredation time
his <- dat[variable == "His", .(sample_id, visit, raw)]
his[sinfo, on = .(sample_id, visit), sample_degredation_time := sample_degredation_time]

g <- ggplot(his, aes(x=sample_degredation_time, y=raw)) +
	geom_hex() +
...
ggsave(g, width=1.55, height=2.2, file="paper_output/his_vs_sample_deg.pdf")


# Plot glycine by well position
gln <- dat[variable == "Gln", .(sample_id, visit, plate_position, plate_row, plate_column, raw)]

# Compute per-well summary
gln <- gln[!is.na(raw), .(metric=names(summary(raw)), value=as.vector(summary(raw))), by=.(plate_position, plate_row, plate_column)]
...
ggsave(g, width=1.3, height=1.4, file="paper_output/gln_well_variation.pdf")
...
ggsave(g, width=1.3, height=1.4, file="paper_output/gln_well_variation_zoom.pdf")
